...
const bestPokemon = () => {
  ...
  try {
    axios
      .get(`https://pokeapi.co/api/v2/pokemon/${bestPokemonIds[randomIndex]}`)
      .then((res) => {
        document.getElementById("shinyPokemon").style.zIndex = 1;
...
const submitHandler = (e) => {
  try {
    ...
    if (searchInput.value.length > 0) {
      axios
        .get(`https://pokeapi.co/api/v2/pokemon/${searchInput.value}`)
        .then((res) => {
          document.getElementById("shinyPokemon").style.zIndex = 1;
...
const opponentPokemon = () => {
  axios
    .get(`https://pokeapi.co/api/v2/pokemon/${Math.floor(Math.random() * 898)}`)
    .then((res) => {
      // console.log(res.data)
...
const battleSubmitHandler = (e) => {
  ...
  if (battleSearchInput.value.length > 0) {
    axios
      .get(`https://pokeapi.co/api/v2/pokemon/${battleSearchInput.value}`)
      .then((res) => {
        name = res.data.species.name;
        name = name.charAt(0).toUpperCase() + name.slice(1);
        yourType = res.data.types[0].type.name;
...
document
  ...
  .addEventListener("click", randomPokemon);
...
document
  .getElementById("opponentPokemonBtn")
  .addEventListener("click", opponentPokemon);
document
  .getElementById("pokemonBattleRandomBtn")
  .addEventListener("click", yourRandomPokemon);
